...
export class UberService {
    ...
    const page = await context.newPage();
                ...
                try {
                    // Navigate to URL
                    const url = process.env.UBERURL;
                    await page.goto(url);
            
                    // Wait for selector and interact with the page
                    ...
                    await inputElement.fill('456 Foothill Boulevard');
                    await page.waitForSelector('ul.cb.h5');
            
                    // Evaluate and click element
                    const clicked = await page.evaluate(() => {
                        const ulElements = Array.from(document.querySelectorAll('ul.cb.h5'));
                    ...
                    if (clicked) {
                        console.log('Clicked on the address');
                    } else {
                        console.log('Failed to click on the address');
                    }
                    ...
                    for (let item of orderDetail.items) {
                        await page.waitForSelector('input[placeholder="Search in Flintridge Pizza Kitchen"]');
